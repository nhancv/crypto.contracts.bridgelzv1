...
contract CrossChainBridgeFull is AccessControlUpgradeable, NonblockingLzAppUpgradeable {
   ...
   */
  function CrossChainBridgeFullInit(address _lzEndpoint) external initializer {
  ...
  function estimateUpdateTextFee(
    ...
    return lzEndpoint.estimateFees(_dstChainId, address(this), lzPayload, _payInZRO, _adapterParam);
  }

  // @notice Send token to destination chain
  // @param _refundAddress - if the source transaction is cheaper than the amount of value passed, refund the additional amount to this address. Ex: payable(_msgSender())
  // @param _zroPaymentAddress - the address of the ZRO token holder who would pay for the transaction. Ex: address(0)
  // @param _adapterParams - parameters for custom functionality. e.g. receive airdropped native gas from the relayer on destination. Ex: "" or bytes memory adapterParams = abi.encode(uint16(1), uint256(200000)); or let adapterParams = ethers.utils.solidityPack(['uint16','uint256'],[1, 200000]);
  function updateText(
    uint16 _dstChainId,
    string memory _text,
    ...
    bytes memory lzPayload = abi.encode(PT_TEXT, _text);
    _lzSend(_dstChainId, lzPayload, payable(_refundAddress), _zroPaymentAddress, _adapterParams, msg.value);
  }
